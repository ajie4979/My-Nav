...
export async function onRequest(context) {
  ...
  const siteName = S.home_site_name || env.SITE_NAME || '简读记忆';
  const siteDescription = S.home_site_description || env.SITE_DESCRIPTION || '一个优雅、快速、易于部署的书签收藏与博客平台';
  ...
  const blogIconHtml = `
    <a href="/blog" class="top-action-icon blog-action-icon" title="博客">
  ...
  const musicPlayerHtml = `
    <div id="musicPlayer" class="relative hidden">
      <button id="musicToggleBtn" class="top-action-icon music-action-icon relative" title="音乐播放器">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3" /></svg>
        <span id="musicPlayingDot" class="absolute -top-0.5 -right-0.5 w-2 h-2 bg-green-500 rounded-full hidden"></span>
      </button>

      <div id="musicPlayerCard" class="hidden absolute top-full right-0 mt-2 w-[231px] bg-white/95 dark:bg-gray-800/95 backdrop-blur rounded-2xl shadow-2xl overflow-hidden z-50">
        <div class="flex items-center justify-between px-2.5 py-1.5 bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
          <div class="flex items-center gap-1">
            <span class="w-1 h-3 bg-green-500 rounded-full"></span>
            <span class="text-[10px] font-semibold text-gray-700 dark:text-gray-200">音乐</span>
          </div>
          <button id="musicCloseBtn" class="w-4 h-4 rounded-full bg-gray-200/70 dark:bg-gray-700 text-gray-500 dark:text-gray-300 flex items-center justify-center hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
        ...
        <div class="flex items-center gap-2.5 px-2.5 pt-2">
          <div id="musicCover" class="w-11 h-11 shrink-0 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center overflow-hidden shadow">
          ...
          <div class="flex-1 min-w-0">
            <h3 id="musicTitle" class="text-xs font-semibold text-gray-800 dark:text-white truncate">未播放</h3>
            <p id="musicArtist" class="text-[10px] text-gray-500 dark:text-gray-400 truncate mt-0.5">--</p>
          </div>
        </div>

        <div class="flex items-center gap-1.5 px-2.5 mt-2">
          <div id="musicProgressBar" class="flex-1 h-1 bg-gray-200 dark:bg-gray-700 rounded-full cursor-pointer overflow-hidden">
            <div id="musicProgress" class="h-full bg-green-500 rounded-full transition-all" style="width: 0%"></div>
          </div>
          <span class="text-[10px] text-gray-400 tabular-nums shrink-0"><span id="musicCurrentTime">0:00</span> / <span id="musicDuration">0:00</span></span>
        </div>

        <div class="flex items-center justify-center gap-2.5 px-2.5 py-1.5">
          <button id="musicShuffleBtn" class="text-gray-500 hover:text-green-500 transition-colors" title="随机播放">
          ...
          <button id="musicPrevBtn" class="text-gray-500 hover:text-green-500 transition-colors" title="上一首">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
              <path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/>
            </svg>
          </button>
          <button id="musicPlayBtn" class="w-7 h-7 rounded-full bg-green-500 text-white flex items-center justify-center hover:bg-green-600 transition-colors shadow" title="播放/暂停">
            <svg id="musicPlayIcon" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
              <path d="M8 5v14l11-7z"/>
            </svg>
          </button>
          <button id="musicNextBtn" class="text-gray-500 hover:text-green-500 transition-colors" title="下一首">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
              <path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/>
            </svg>
          </button>
          <button id="musicListBtn" class="text-gray-500 hover:text-green-500 transition-colors" title="播放列表">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex items-center gap-1.5 px-2.5 pb-2">
          ...
          <input type="range" id="musicVolume" min="0" max="100" value="70" class="flex-1 h-1 bg-gray-200 dark:bg-gray-700 rounded-full appearance-none cursor-pointer">
        </div>

        <div id="musicListPanel" class="hidden border-t border-gray-100 dark:border-gray-700 max-h-[130px] overflow-y-auto px-1.5 py-1.5">
          <div id="musicListContainer" class="space-y-0.5"></div>
        </div>
      </div>
    </div>`;

  const topRightActionsHtml = `<div class="fixed top-4 right-4 z-50 flex items-center gap-3">${musicPlayerHtml}${themeIconHtml}${blogIconHtml}${adminIconHtml}</div>`;
  ...
  const cardHydrationState = buildCardHydrationState(allSites, S, env.ICON_API);
  ...
  const replacements = {
    ...
    'HEADER_EXTRA_CLASS': headerExtraClass,
