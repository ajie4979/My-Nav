// functions/admin/posts.js
// 文章管理页面 - 服务端渲染，注入 CSRF token
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    url.pathname = '/admin/posts-page.html';
    ...
    if (response.ok) {
      ...
      console.error('Failed to load admin posts HTML:', response.status);
      return new Response('文章管理页面加载失败', { status: 500 });
    }
  } catch (e) {
    console.error('Error loading admin posts page:', e);
    return new Response('文章管理页面加载失败: ' + e.message, { status: 500 });
  }
}
