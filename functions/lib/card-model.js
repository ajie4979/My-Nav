import { escapeHTML, sanitizeUrl, buildFaviconUrl } from './utils';
...
export function buildCardViewModel(site, iconAPI) {
  const rawName = site?.name || '未命名';
  const normalizedUrl = sanitizeUrl(site?.url);
  const normalizedLogo = buildFaviconUrl(site?.url, site?.logo, iconAPI);
...
export function buildCardHydrationState(sites, settings = {}, iconAPI) {
  return {
    ...
    cards: (sites || []).map(site => buildCardViewModel(site, iconAPI)),
  };
}
