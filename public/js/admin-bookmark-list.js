(function () {
  ...
  function getLogoHtml(config, safeName, cardInitial) {
    ...
    // 无 logo：尝试按域名自动获取 favicon，失败时回退为首字母
    const url = window.normalizeUrl(config.url);
    if (url && /^https?:\/\//i.test(url)) {
      try {
        const domain = new URL(url).host;
        const faviconUrl = `${ADMIN_ICON_API}${encodeURIComponent(domain)}`;
        // onerror 内联回退：内部 class 双引号需转义为 &quot;，避免截断 onerror 属性
        const fallbackHtml = `<div class=&quot;w-full h-full rounded-lg bg-primary-100 text-primary-600 flex items-center justify-center font-bold text-lg&quot;>${cardInitial}</div>`;
        return `<img src="${window.escapeHTML(faviconUrl)}" alt="${safeName}" class="w-full h-full rounded-lg object-cover bg-gray-50" loading="lazy" onerror="this.outerHTML='${fallbackHtml}'">`;
      } catch (e) {
        // fall through
      }
    }
      ...
      <div class="p-5">
        <div class="block">
          <div class="flex items-start">
            <div class="site-icon flex-shrink-0 mr-4 w-10 h-10">
              ${logoHtml}
            </div>
