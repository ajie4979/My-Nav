(function () {
  ...
  Home.initSearch = function () {
    ...
    function updateHeading(keyword, activeCatalog, count) {
      ...
      if (!heading) return;
      const headingTextEl = heading.querySelector('#listHeadingText') || heading;
      ...
      if (keyword) {
        headingTextEl.textContent = isMobile ? `${visibleCount} 个书签` : `搜索结果 · ${visibleCount} 个书签`;
      } else {
        const currentActive = heading.dataset.active;
        if (isMobile) {
          headingTextEl.textContent = `${visibleCount} 个书签`;
        } else if (currentActive) {
          headingTextEl.textContent = `${currentActive} · ${visibleCount} 个书签`;
        } else {
          headingTextEl.textContent = `全部收藏 · ${visibleCount} 个书签`;
        }
      }
    }
